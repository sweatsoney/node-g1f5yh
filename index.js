...
(() => {
  ...
  if (window['UltraTypeCore']) {
    ...
    console.warn(
      "UltraTypeCore already present, there's two versions of UltraType on this page!"
    );
    return;
  }
  // Constants
  ...
  const _type = (charCode) => {
    ...
    turbo = () => {
      debug(
        'Turbo mode called. Sending ' +
          TURBO_PACKET_COUNT.toString() +
          ' type packets.'
      );
    ...
    addGraph = (g) => {
      ...
      if (root) {
        let _style = $(
          '<style>.highcharts-container{width:100% !important;height:100% !important;display:inline-block;}</style>'
        );
    ...
    checkIfBanned = (callback) => {
      if (userInfo.username) {
        ...
        debug(
          "WARN: Can't check if my user is banned, the userInfo is not valid:",
          userInfo
        );
    },
    updateStats = () => {
      if (userInfo.username) {
        ...
        stm1.textContent =
          '$' + userInfo.money + ' (Spent: $' + userInfo.moneySpent + ')';
    ...
    randomBool = (percentFalse) => {
      ...
      if (typeof percentFalse === 'number') {
        ...
        debug(
          'WARN: No percentage false specified for random boolean generation. Using 0.5.'
        );
      }
      ret = Math.random() > percent;
      tdebug(
        'Calculated random bool with false percentage',
        percent,
        'Result:',
        ret
      );
      ...
      */
      ...
      setTimeout(() => {
        ...
        if (enabled) {
          ...
          if (autoNitro) {
            if (randomBool(0.999)) {
              // Theres a 0.1% chance that a nitro is used mid race during a tick
    ...
    lessonLoad = () => {
      debug(
        'The prerendered lesson has been captured and loaded. Starting in ' +
          LOAD_TIME / 1000 +
          ' seconds.'
      );
      if (!isStopped) {
    ...
    respawn = () => {
      ...
      setTimeout(location.reload.bind(location), gen(750, 1100));
    },
    removeUITrash = () => {
    ...
    createUI = (body) => {
      ...
      enableButton.style.transition =
        'border 500ms, border-color 500ms, color 500ms';
      ...
      turboBtn.onclick = () => {
        ...
        if (!firstTurbo) {
          ...
          if (localStorage['turboAlert']) {
            ...
            alert(
              'WARNING: Abuse of turbo mode may get you banned!\nThis message will not be displayed again.'
            );
    ...
    createOptsMenu = () => {
      ...
      wpm.onchange = () => {
        if (localStorage['speedChange']) {
          ...
          if (isNaN(wordsPerMinute)) wpm.value = 85;
          setWPM(wpm.value);
        } else {
    ...
    changeTip = (node) => {
      setTimeout(() => {
        ...
        node.innerHTML += FONT;
        node.innerHTML +=
          '<center style="font-family:Ubuntu;">UltraType - NitroType simplified.<br>Version: ' +
          VERSION +
          '</center>';
      }, 1000);
    },
  ...
  XMLHttpRequest.prototype.open = function () {
    if (arguments[1].includes('/api/error')) {
      ...
      if (ABORT_PROBLEM_KEYS) {
        ...
        debug(
          "Detected outgoing problem-keys AJAX request, but ABORT_PROBLEM_KEYS is false, so I'm letting it send."
        );
      }
    }
  ...
  Function.prototype.toString = __.toStr = function () {
    ...
    if (this === CanvasRenderingContext2D.prototype.fillText)
      return _.toStr.call(_.fill);
    if (this === Object.prototype.__lookupGetter__) return _.toStr.call(_.get);
    if (this === ShadowRoot.prototype.__lookupGetter__('host'))
      return _.toStr.call(hostt);
    if (this === Function.prototype.__lookupGetter__('toString'))
      return _.toStr.call(_getToStr);
    if (this === Element.prototype.__lookupSetter__('textContent'))
      return _.toStr.call(_setTxt);
    if (this === Document.prototype.__lookupGetter__('title'))
      return _.toStr.call(_getTitle);
    if (this === Document.prototype.__lookupSetter__('title'))
      return _.toStr.call(_setTitle);
    ...
    if (window.jQuery && this === jQuery.fn.keypress)
      return _.toStr.call(_attachHandler);
  ...
  Function.prototype.__defineGetter__('toString', function () {
    if (
      this === CanvasRenderingContext2D.prototype ||
      this === CanvasRenderingContext2D.prototype.fillText
    )
      return __.toStr;
    if (this === console || this === console.warn) return __.toStr;
    if (
      this === ShadowRoot.prototype.__lookupGetter__('host') ||
      this === ShadowRoot.prototype
    )
      return __.toStr;
    if (this === Object.prototype || this === Object.prototype.__lookupGetter__)
      return __.toStr;
    if (this === Function.prototype.__lookupGetter__('toString'))
      return __.toStr;
  ...
  */
  ...
  let core = {
    ...
    typeChar: type,
    stopFromRunning: () => {
      // Stops the bot from appearing or typing
      isStopped = true;
    },
    getDecyptedUserInfo: () => {
      if (userInfo) {
